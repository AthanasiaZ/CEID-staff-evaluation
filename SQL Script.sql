...
CREATE INDEX `EMPLFIRM` ON `CEID-Staff-Evaluation`.`employee` (`firm` ASC) VISIBLE;
...
CREATE INDEX `HASDEGRINST` ON `CEID-Staff-Evaluation`.`has_degree` (`institution` ASC) VISIBLE;

CREATE INDEX `HASDEGREMPL` ON `CEID-Staff-Evaluation`.`has_degree` (`emplUsername` ASC) VISIBLE;
...
CREATE INDEX `PROJEMPL` ON `CEID-Staff-Evaluation`.`project` (`employee` ASC) VISIBLE;
...
CREATE INDEX `JOBEVAL` ON `CEID-Staff-Evaluation`.`job` (`evaluator` ASC) VISIBLE;

CREATE INDEX `JOBEDRA` ON `CEID-Staff-Evaluation`.`job` (`edra` ASC) VISIBLE;
...
CREATE INDEX `NEEDSANTIKEIM` ON `CEID-Staff-Evaluation`.`needs` (`fieldTitle` ASC) VISIBLE;
...
CREATE INDEX `REQEVALJOB` ON `CEID-Staff-Evaluation`.`request_evaluation` (`jobID` ASC) VISIBLE;
...
CREATE INDEX `EVALEMPL` ON `CEID-Staff-Evaluation`.`evaluation` (`emplUsername` ASC) VISIBLE;

CREATE INDEX `EVALJOB` ON `CEID-Staff-Evaluation`.`evaluation` (`jobID` ASC) VISIBLE;
...
CREATE INDEX `LANGEMPL` ON `CEID-Staff-Evaluation`.`languages` (`employee` ASC) VISIBLE;
...
USE `CEID-Staff-Evaluation`$$
DROP TRIGGER IF EXISTS `CEID-Staff-Evaluation`.`manager_AFTER_INSERT` $$
USE `CEID-Staff-Evaluation`$$
CREATE DEFINER = CURRENT_USER TRIGGER `CEID-Staff-Evaluation`.`manager_AFTER_INSERT` AFTER INSERT ON `manager` FOR EACH ROW
BEGIN
